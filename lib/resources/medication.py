...
def generate_medication_request_custom(patient_id: str, practitioner_id: str, medication_name: str) -> Dict[str, Any]:
    """
    Generates a single FHIR MedicationRequest resource with a specific medication.

    :param patient_id: The ID of the patient for the medication request.
    :param practitioner_id: The ID of the practitioner who prescribed the medication.
    :param medication_name: The name of the medication to prescribe.
    :return: A dictionary representing the FHIR MedicationRequest resource.
    """
    medication_request_id = str(uuid.uuid4())
    
    # Find the medication in MEDICATIONS list
    medication = None
    ...
    # If not found, create a default medication entry
    if not medication:
        medication = {
